...
(books_list <- list.files(path = "docs/data/dickens", full.names = TRUE))

# convert a list of text files 
books <- readtext(books_list)
class(books)
# Notice this will save as a readtext dataframe object. Let's convert to our 
# tidyverse's format for dataframes, called a tibble.
books <- as_tibble(books)

# now have all texts in one dataframe. We can extract metadata from the filenames
...
## Reminder: There are R packages for many document databases such as Project Gutenberg https://cran.r-project.org/web/packages/gutenbergr/vignettes/intro.html.
